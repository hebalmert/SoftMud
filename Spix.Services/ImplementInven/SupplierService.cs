...
public class SupplierService : ISupplierServices
{
    ...
    public async Task<ActionResponse<IEnumerable<Supplier>>> ComboAsync(string email)
    {
        try
        {
    ...
    public async Task<ActionResponse<Supplier>> UpdateAsync(Supplier modelo, string frontUrl)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(modelo.ImgBase64))
            {
                ...
                NewModelo.Photo = await _fileStorage.UploadImage(imageId, _imgOption.ImgSuppliers!, guid);
            }
    ...
    public async Task<ActionResponse<Supplier>> AddAsync(Supplier modelo, string email, string frontUrl)
    {
        await _transactionManager.BeginTransactionAsync();
        try
        {
            ...
            if (!string.IsNullOrEmpty(modelo.ImgBase64))
            {
                ...
                var imageId = Convert.FromBase64String(modelo.ImgBase64);
                modelo.Photo = await _fileStorage.UploadImage(imageId, _imgOption.ImgSuppliers!, guid);
            }
    ...
    public async Task<ActionResponse<bool>> DeleteAsync(Guid id)
    {
        ...
        try
        {
            ...
            {
                var response = _fileStorage.DeleteImage(_imgOption.ImgSuppliers!, DataRemove.Photo);
                if (!response)
                {
                    return new ActionResponse<bool>
                    {
