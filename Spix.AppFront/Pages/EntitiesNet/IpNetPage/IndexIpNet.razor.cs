...
public partial class IndexIpNet
{
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<IpNet>>(url);
    ...
    private async Task ShowModalAsync(Guid? id = null, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await _dialogService.ShowAsync<EditIpNet>($"Editar Ip", parameters, options);
        }
        else
        {
            dialog = await _dialogService.ShowAsync<CreateIpNet>($"Nueva Ip", options);
        }
