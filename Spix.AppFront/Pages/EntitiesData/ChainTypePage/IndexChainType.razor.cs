...
public partial class IndexChainType
{
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<ChainType>>(url);
        ...
        ChainTypes = responseHttp.Response;
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await _dialogService.ShowAsync<EditChainType>($"Editar Chain", parameters, options);
        }
        else
        {
            dialog = await _dialogService.ShowAsync<CreateChainType>($"Nuevo Chain", options);
        }
