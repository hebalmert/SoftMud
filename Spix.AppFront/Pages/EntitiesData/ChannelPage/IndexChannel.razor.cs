...
public partial class IndexChannel
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await _dialogService.ShowAsync<EditChannel>($"Editar Canal", parameters, options);
        }
        else
        {
            dialog = await _dialogService.ShowAsync<CreateChannel>($"Nuevo Canal", options);
        }
