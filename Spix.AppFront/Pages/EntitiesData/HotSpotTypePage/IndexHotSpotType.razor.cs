...
public partial class IndexHotSpotType
{
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<HotSpotType>>(url);
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await _dialogService.ShowAsync<EditHotSpotType>($"Editar HotSpot Tipo", parameters, options);
        }
        else
        {
            dialog = await _dialogService.ShowAsync<CreateHotSpotType>($"Nuevo HotSpot Tipo", options);
        }
