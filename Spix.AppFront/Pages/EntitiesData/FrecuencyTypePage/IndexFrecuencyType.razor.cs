...
public partial class IndexFrecuencyType
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await _dialogService.ShowAsync<EditFrecuencyType>($"Editar Tipo Frecuencia", parameters, options);
        }
        else
        {
            dialog = await _dialogService.ShowAsync<CreateFrecuencyType>($"Nuevo Tipo Frecuencia", options);
        }
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<FrecuencyType>>(url);
