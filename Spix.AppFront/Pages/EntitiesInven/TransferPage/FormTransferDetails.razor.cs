...
public partial class FormTransferDetails
{
    ...
    {
        TransferDetails.ProductCategoryId = modelo.ProductCategoryId;
    ...
    private async Task ProductsChanged(Product modelo)
    {
        ...
        var responseHTTP = await _repository.GetAsync<TransferStockDTO>($"api/v1/productStocks/transferStock?TransferId={TransferDetails.TransferId}&ProductId={modelo.ProductId}");
        // Centralizamos el manejo de errores
        bool errorHandled = await _responseHandler.HandleErrorAsync(responseHTTP);
        if (errorHandled)
        {
