...
public class ServiceCategoriesController : ControllerBase
{
    ...
    public ServiceCategoriesController(IServiceCategoryUnitOfWork serviceCategoryUnitOfWork)
    {
    ...
    public async Task<ActionResult<IEnumerable<ServiceCategory>>> GetAll([FromQuery] PaginationDTO pagination)
    {
        ...
        var response = await _serviceCategoryUnitOfWork.GetAsync(pagination, email);
        if (!response.WasSuccess)
        {
            return BadRequest(response.Message);
        }
    ...
    {
        var response = await _serviceCategoryUnitOfWork.GetAsync(id);
        if (response.WasSuccess)
        {
            return Ok(response.Result);
        }
    ...
    [HttpPut]
    public async Task<ActionResult<ServiceCategory>> PutAsync(ServiceCategory modelo)
    {
        var response = await _serviceCategoryUnitOfWork.UpdateAsync(modelo);
        if (response.WasSuccess)
        {
            return Ok(response.Result);
        }
    ...
    [HttpPost]
    public async Task<ActionResult<ServiceCategory>> PostAsync(ServiceCategory modelo)
    {
        string email = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name)!.Value;
        if (email == null)
        {
        ...
        var response = await _serviceCategoryUnitOfWork.AddAsync(modelo, email);
        if (response.WasSuccess)
        {
            return Ok(response.Result);
        }
    ...
    {
        var response = await _serviceCategoryUnitOfWork.DeleteAsync(id);
        if (response.WasSuccess)
        {
            return Ok(response.Result);
        }
